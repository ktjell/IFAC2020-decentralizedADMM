...
#                    print('RES', RES)
    ...
    def calCliqSum(self,val):
        ...
        for i in self.cliques[self.i]:
            ...
            for j in i[1:]:  
                res1.append(self.get_val('r'+str(i[0])+str(j)))
        ...
        for i in self.cliques[self.i]:
            res1 = []
            for j in i[1:]: 
                res1.append(self.get_val('val'+str(i[0])+str(j)))
...
time_ar = []
ITE=10
links_ar = [20+10*i for i in range(ITE)]
timeMAX = []
timeMIN = []
...
for i in range(ITE):
    ...
    for ii in range(1):
        neighbors, cliques, vircliques, w, edges, links = graph.get_graph_info(n,links_ar[i])
#        print(type(cliques))
#        np.savetxt('CLIQUES.out', np.array(cliques))
        threads = []
        for k in range(n):
            
            threads.append(party(F,np.array([np.random.randint(100), np.random.randint(100)]),n,t1,k,q[k],com,E[k], Q, f[k], retur, g, ite, neighbors, cliques, vircliques, w))
