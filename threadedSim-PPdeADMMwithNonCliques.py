...
startsted = 1
slutsted = 2
ITE = slutsted - startsted
AVRAGE = 2
links_ar =[n*i for i in range(startsted,slutsted)]
f = []
for k in range(n):
...
#    LINKS +=10
    timetemp = []
    for i in range(AVRAGE):
        neighbors, cliques, vircliques, w, edges, links = graph.get_graph_info(n,links_ar[ii])
        
        threads = []
        for k in range(n):
            
            threads.append(party(F,np.array([np.random.randint(100), np.random.randint(100)]),n,t,k,q[k],com,E[k], Q, f[k], retur, g, ite, neighbors, cliques, vircliques, w))
...
print('\n MAX')
for num,k in enumerate(timetemp_array):
    print('({},{})'.format(num+startsted, np.max(k)))
 
print('\n MIN')
for num,k in enumerate(timetemp_array):
    print('({},{})'.format(num+startsted, np.min(k)))

print('\n MEAN')

for num,k in enumerate(timetemp_array):
    print('({},{})'.format(num+startsted, np.mean(k)))
#plt.figure('1')

#plt.figure()
#plt.plot(links_ar, tarray)
#plt.plot(links_ar, timeMAX)
#plt.plot(links_ar, timeMIN)
